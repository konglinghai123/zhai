 ...
 */
public class BeautifyLegRecyclerViewAdaper extends BaseRecyclerViewAdapter<BeautyLegListBean> {
    public BeautifyLegRecyclerViewAdaper(Context context){
        super(context);
    }
    @Override
    public int headinsert(List<BeautyLegListBean> list) {
        int count=0;
        if(this.data.size()>0){
            for (BeautyLegListBean galleryBean:list){
                for (int i=0;i<this.data.size();i++){
                    if(galleryBean.getTerm().equals(this.data.get(i).getTerm())){
                        break;
    ...
    public String showImage(int position) {
        return data.get(position).getImg();
    }

    @Override
    public void ImageOnClickListener(int position) {
        BeautyLegListBean bean = data.get(position);
